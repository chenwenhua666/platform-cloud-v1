...
import com.plm.platform.common.entity.system.Role;
import com.plm.platform.common.utils.PlatformUtil;
import com.plm.platform.server.system.service.IRoleService;
...
public class RoleController {
    ...
    @GetMapping
    public PlatformResponse roleList(QueryRequest queryRequest, Role role) {
        Map<String, Object> dataTable = PlatformUtil.getDataTable(roleService.findRoles(role, queryRequest));
        return new PlatformResponse().data(dataTable);
    }

    @GetMapping("options")
    public PlatformResponse roles() {
        List<Role> allRoles = roleService.findAllRoles();
        return new PlatformResponse().data(allRoles);
    }
